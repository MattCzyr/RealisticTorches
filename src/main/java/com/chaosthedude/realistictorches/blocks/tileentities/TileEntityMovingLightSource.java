...
public class TileEntityMovingLightSource extends TileEntity {
	...
	@Override
	public boolean shouldRefresh(Block oldB, Block newB, int oldMeta, int newMeta, World world, int x, int y,
			int z) {
	...
	public void updateEntity() {
		EntityPlayer player = worldObj.getClosestPlayer(this.xCoord + 0.5D, this.yCoord + 0.5D, this.zCoord + 0.5D,
				2.0D);
		if (player == null) {
		...
		try {
			if (!BlockMovingLightSource.isLightEmittingItem(player.getCurrentEquippedItem().getItem())
					&& player.getCurrentEquippedItem().getItem() != Item.getItemFromBlock(BlockRegistry.torchLit)) {
				if (worldObj.getBlock(xCoord, yCoord, zCoord) == BlockRegistry.movingLightSource) {
					worldObj.setBlockToAir(xCoord, yCoord, zCoord);
				}
			}
